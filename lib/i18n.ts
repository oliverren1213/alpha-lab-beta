...
const englishByChinese: Record<string, string> = {
  "先选语言": "Choose your language", "Public Beta 默认以英文打开。右上角的“中文 / EN”按钮可以随时切换，页面内容会立即同步。": "Public Beta always opens in English. Use the “中文 / EN” control in the top-right to switch the entire interface at any time.",
  "先读懂组合": "Read the portfolio", "从账户净值、今日影响和累计损益开始，再向下核对 NVDA 100 股、GOOGL 25 股与 $1,000 黄金仓位的模拟成本口径。": "Start with portfolio value, today’s impact and cumulative P&L, then trace the simulated cost basis of 100 NVDA shares, 25 GOOGL shares and a $1,000 gold allocation.",
  "再追溯每笔交易": "Trace every trade", "账本是所有计算的源头。试着新增或编辑一笔模拟交易；重复成交会被自动识别，不会悄悄写入两次。": "The ledger drives every calculation. Add or edit a simulated trade; duplicate executions are detected before they can be recorded twice.",
  "检查当初为什么买": "Review the original thesis", "阅读预置的 NVDA 与 GOOGL 投资逻辑，再追加 revision。新判断只会追加，不会覆盖原始理由。": "Read the sample NVDA and GOOGL theses, then append a revision. New evidence is added without rewriting the original rationale.",
  "最后做压力测试": "Run a stress test", "调整 QQQ、半导体或单一标的冲击，观察组合影响。情景结果不会改动正式账本。": "Adjust QQQ, semiconductor or asset-specific shocks and inspect the portfolio impact. Scenario results never alter the ledger.",
  "知道每个数字从哪来": "Know where every number came from", "演示价格是固定模拟值，不是实时行情。正式数据模式会保留价格日期、来源、报价时间与延迟状态。": "Demo prices are fixed simulations, not live quotes. Production data keeps the price date, provider, quote time and delay status visible.",
  "成本、收益与仓位": "Cost, returns and exposure", "录入、编辑与去重": "Add, edit and deduplicate",
  "原始理由与 revision": "Original rationale and revisions", "冲击而不改账本": "Stress test without changing the ledger",
  "来源、时间与延迟": "Source, timestamp and delay", "关闭指南": "Close guide", "指南进度": "Guide progress",
  "所有持仓、价格与投资逻辑均为模拟示例，不构成投资建议。": "All holdings, prices and investment theses are simulated examples—not investment advice.",
  "模拟组合预览": "Simulated portfolio preview", "两次买入，展示加权成本": "Two entries demonstrate weighted cost",
  "长期逻辑示例": "Long-horizon thesis example", "黄金初始投入": "Initial gold allocation",
  "上一步": "Back", "下一步": "Next", "进入模拟组合": "Enter the demo portfolio", "打开这一页": "Open this section", "界面语言": "Interface language",
  "演示快捷指南": "Demo quick guide", "数据说明": "Data notes", "了解模拟数据": "About demo data",
  "模拟快照": "Simulated snapshot", "固定情景数据 · 非实时行情": "Fixed scenario data · not live market quotes",
  "演示空间使用固定且明确标注的模拟价格，不会伪装成实时行情。": "The demo uses fixed, clearly labeled simulated prices and never presents them as live quotes.",
  "AI 基础设施需求可能持续复合增长，而软件生态进一步强化转换成本。": "AI infrastructure demand can compound while the software ecosystem reinforces switching costs.",
  "该逻辑认为，平台经济性的持续性比单季度 headline growth 更重要。": "The thesis assumes durable platform economics matter more than one quarter of headline growth.",
  "新架构采用；数据中心需求；持续稳固的开发者生态。": "New architecture adoption; data-center demand; sustained developer ecosystem.",
  "客户集中；供应限制；竞争；估值压缩。": "Customer concentration; supply constraints; competition; valuation compression.",
  "若数据中心需求连续两个周期走弱，或平台相关性出现结构性下降，则逻辑失效。": "Two consecutive periods of weakening data-center demand or a structural loss of platform relevance.",
  "长期逻辑仍在，但仓位必须体现半导体周期性。": "The long-term thesis is intact, but position sizing must reflect semiconductor cyclicality.",
  "只有需求能够转化为现金流，经营杠杆才有意义。": "Operating leverage remains meaningful only if demand converts into cash flow.",
  "产品路线图执行与更广泛的推理需求。": "Execution against product roadmap and broadening inference demand.",
  "更高波动以及超预期的支出放缓。": "Higher volatility and a sharper-than-expected spending slowdown.",
  "若路线图执行延迟且客户资本开支同步走弱，应降低信心。": "Reduce conviction if roadmap execution slips while customer capex also weakens.",
  "搜索业务现金流可为云与 AI 投资提供资金，同时业务保持全球多元化。": "Search cash generation can fund cloud and AI investment while the business remains globally diversified.",
  "该演示逻辑认为市场低估了核心现金流引擎的持续性。": "The demo thesis assumes the market underestimates the durability of the core cash engine.",
  "云业务利润率扩张；AI 产品变现；纪律化资本配置。": "Cloud margin expansion; AI product monetization; disciplined capital allocation.",
  "搜索颠覆；监管；基础设施成本上升；云业务执行放缓。": "Search disruption; regulation; rising infrastructure costs; slower cloud execution.",
  "若核心搜索经济性恶化且云业务现金流无法抵消，则重新评估。": "Reassess if core search economics deteriorate without offsetting cloud cash flow.",
  "示意性公开信息 · 仅用于演示": "Illustrative public information · demo only",
  "示例投资逻辑 · 不构成投资建议": "Illustrative thesis · not investment advice",
  "示例 revision：更新证据，但不改写原始逻辑。": "Example revision: update the evidence without rewriting the original thesis.",
  "引导式演示组合": "Guided demo portfolio",
  "NVIDIA · 模拟持仓": "NVIDIA · simulated holding",
  "Alphabet A 类 · 模拟持仓": "Alphabet Class A · simulated holding",
  "SPDR 黄金 ETF · 模拟持仓": "SPDR Gold Shares · simulated holding",
  "模拟演示 · 非真实持仓或市场报价": "SIMULATED DEMO · not a real holding or market quote",
  "Alpha Lab 模拟演示": "Alpha Lab simulated demo",
  "Alpha Lab 模拟演示 · 非实时": "Alpha Lab simulated demo · not live",
  ...
  "现金余额": "Cash balance", "未追踪": "Not tracked", "部分追踪": "Partially tracked", "现金余额为负": "Negative cash balance",
  "现金未追踪": "Cash not tracked", "现金部分追踪": "Cash partially tracked",
  "只记录成交时，Alpha Lab 不会凭空推算负现金；组合净值按可估值持仓计算。": "When only trades are recorded, Alpha Lab does not invent a negative cash balance; portfolio value uses holdings that can be valued.",
  "按当前可估值资产计算；现金只在账本存在明确转入、转出记录时计入。": "Calculated from currently valued assets; cash is included only when explicit transfers exist in the ledger.",
  "SMART CAPTURE": "SMART CAPTURE", "上传成交截图，自动填表": "Upload a trade screenshot to prefill the form",
  "图片只在你的浏览器内识别，不上传、不保存、无需 API key。结果必须由你核对，重复成交仍会被拦截。": "Recognition runs entirely in your browser. Images are never uploaded or stored, and no API key is needed. Review every result; duplicates remain blocked.",
  "选择交易截图": "Choose trade screenshot", "JPEG、PNG 或 WebP · 最大 8 MB": "JPEG, PNG or WebP · 8 MB max", "识别交易": "Recognize trades",
  "首次使用会下载中英文字库，通常需要几秒；模糊字段会留给你确认，不会自动入账。": "The first run downloads English and Chinese OCR data and may take a few seconds. Ambiguous fields stay blank for review and are never posted automatically.",
  "核对并填入": "Review and prefill", "未匹配标的": "Unmatched asset", "日期待确认": "Date to confirm", "数量待确认": "Quantity to confirm", "价格待确认": "Price to confirm", "账户待确认": "Account to confirm",
